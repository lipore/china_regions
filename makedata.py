    ...
    for k in list(reversed(source_file_list))[0:-2]:
        ...
        if k == 'province':
            for index, i in enumerate(json_data):
                mysql_data = "INSERT INTO province (`name`, `province_id`) VALUES ('%s', '%s');\n" % (i['name'], i['id'])  # noqa
        ...
        if k == 'city':
            ...
            for province_id in sorted(json_data.keys()):
                for city in json_data[province_id]:
                    index += 1
                    mysql_data = "INSERT INTO city (`name`, `city_id`, `province_id`) VALUES ('%s', '%s', '%s');\n" % (city['name'], city['id'], province_id)  # noqa
                    mysql_data_list.append(mysql_data)
        ...
        if k == 'county':
            ...
            for city_id in sorted(json_data.keys()):
                for county in json_data[city_id]:
                    index += 1
                    mysql_data = "INSERT INTO county ( `name`, `county_id`, `city_id`) VALUES ('%s', '%s', '%s');\n" % (county['name'], county['id'], city_id)  # noqa
                    mysql_data_list.append(mysql_data)
        ...
        if k == 'town':
            ...
            for county_id in sorted(json_data.keys()):
                for town in json_data[county_id]:
                    index += 1
                    mysql_data = "INSERT INTO town ( `name`, `town_id`, `county_id`) VALUES ('%s', '%s', '%s');\n" % (town['name'], town['id'], county_id)  # noqa
                    mysql_data_list.append(mysql_data)
        ...
        if k == 'village':
            ...
            for town_id in sorted(json_data.keys()):
                for village in json_data[town_id]:
                    index += 1
                    mysql_data = "INSERT INTO village ( `name`, `village_id`, `town_id`) VALUES ('%s', '%s', '%s');\n" % (village['name'], village['id'], town_id)  # noqa
                    mysql_data_list.append(mysql_data)
